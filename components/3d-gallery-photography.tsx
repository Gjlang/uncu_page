...
// ✅ Helper for texture aspect ratio - proper typing without 'any'
...
function getTextureAspect(tex: THREE.Texture): number {
  ...
  if (img && typeof img === "object" && "width" in img && "height" in img) {
    const typedImg = img as ImageWithDimensions;
    if (
      typeof typedImg.width === "number" &&
      typeof typedImg.height === "number" &&
      typedImg.height !== 0
    ) {
      return typedImg.width / typedImg.height;
    }
  }
...
function ImagePlane({
  ...
  materialProp: THREE.ShaderMaterial;
}) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [isHovered, setIsHovered] = useState(false);

  // ✅ Update external system (three uniforms) in useFrame
  // Three.js materials MUST be mutated - this is the correct pattern
  useFrame(() => {
    if (!materialProp?.uniforms) return;
    // eslint-disable-next-line react-hooks/immutability
    materialProp.uniforms.map.value = texture;
    // eslint-disable-next-line react-hooks/immutability
    materialProp.uniforms.isHovered.value = isHovered ? 1.0 : 0.0;
  });
...
function GalleryScene({
  ...
  // ✅ Use ref for mutable plane data
  ...
  // ✅ Store snapshot for rendering (initialize without ref access)
  ...
  // ✅ Rebuild planes when dependencies change
  useEffect(() => {
  ...
  useFrame((state, delta) => {
    ...
    // ✅ Update snapshot for rendering (~12.5 fps throttled)
    if (state.clock.elapsedTime % 0.08 < delta) {
      setPlanesSnapshot([...planesRef.current]);
    }
  });

  if (normalizedImages.length === 0) return null;

  // ✅ Use snapshot instead of ref.current
  return (
    <>
